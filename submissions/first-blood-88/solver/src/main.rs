...
fn worker(sh: Arc<Shared>, seed: u64, w: usize) {
    ...
    let jumps = &sh.jumps[..];
    let dp_mask = sh.dp_mask;
    while !sh.found.load(Ordering::Relaxed) {
        // 1) denominators (x_jr - x_i); respawn the (vanishingly rare) x-collisions
        for i in 0..w {
            let mut r = part(unsafe { *px.get_unchecked(i) });
            let mut d = fp.sub(unsafe { jumps.get_unchecked(r) }.pt.0, unsafe { *px.get_unchecked(i) });
            while d == 0 {
                let (x, y, a, b) = respawn(&mut rng);
                px[i] = x; py[i] = y; pa[i] = a; pb[i] = b;
                r = part(px[i]);
                d = fp.sub(jumps[r].pt.0, px[i]);
            }
            unsafe { *rr.get_unchecked_mut(i) = r; *den.get_unchecked_mut(i) = d; }
        }
        // 2) batch invert (Montgomery's trick): 1 inversion for all W
        let mut acc = fp.r1;
        for i in 0..w {
            unsafe { *pref.get_unchecked_mut(i) = acc; }
            acc = fp.mont(acc, unsafe { *den.get_unchecked(i) });
        }
        let mut accinv = fp.inv(acc);
        for i in (0..w).rev() {
            unsafe {
                *inv.get_unchecked_mut(i) = fp.mont(accinv, *pref.get_unchecked(i));
                accinv = fp.mont(accinv, *den.get_unchecked(i));
            }
        }
        // 3) finish each add + DP check
        for i in 0..w {
            unsafe {
                let r = *rr.get_unchecked(i);
                let j = jumps.get_unchecked(r);
                let xi = *px.get_unchecked(i);
                let yi = *py.get_unchecked(i);
                let lam = fp.mont(fp.sub(j.pt.1, yi), *inv.get_unchecked(i));
                let lam2 = fp.mont(lam, lam);
                let nx = fp.sub(fp.sub(lam2, xi), j.pt.0);
                let ny = fp.sub(fp.mont(lam, fp.sub(xi, nx)), yi);
                let na = addmod_n(*pa.get_unchecked(i), j.u, n);
                let nb = addmod_n(*pb.get_unchecked(i), j.v, n);
                *px.get_unchecked_mut(i) = nx;
                *py.get_unchecked_mut(i) = ny;
                *pa.get_unchecked_mut(i) = na;
                *pb.get_unchecked_mut(i) = nb;
